...
const FIELD_LABELS := {
	...
	"device": "Συσκευή χορήγησης O₂",
	"fiO2_setting": "Ρύθμιση FiO₂ (%)",
	"flow_rate": "Ροή (L/min)",
...
func setup(ehr_config: Dictionary) -> void:
	...
	for form_id in forms:
		...
		var title := str(form.get("title", form_id))
...
func _make_input(field: String) -> Control:
	...
	match field:
		"consciousness":
			out = _option(["", "Εγρήγορση (A)", "Απόκριση σε φωνή (V)", "Απόκριση σε πόνο (P)", "Καμία απόκριση (U)"])
		"skin_color":
